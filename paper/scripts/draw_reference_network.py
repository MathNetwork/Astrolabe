...
def generate_tikz(nodes, edges, positions, scale=1.0):
    """Generate TikZ code from layout positions."""
    lines = []
    lines.append("\\begin{tikzpicture}[")
    lines.append("  every node/.style={circle, draw, inner sep=1pt, font=\\tiny\\ttfamily,")
    lines.append("    minimum size=14pt, fill=white},")
    lines.append("  every edge/.style={->, >=Stealth, thin}]")

    for h, (x, y) in positions.items():
        sx, sy = x * scale, y * scale
        lines.append(f"  \\node ({h}) at ({sx:.2f},{sy:.2f}) {{{h}}};")
