...
The disk format is \texttt{astrolabe.json}, graded by dimension as defined in \S\ref{sec:foundations}. In memory, the complex is loaded into a Hasse diagram with four indices:
...
Key operation complexities: node lookup $O(1)$, restriction map $d_i$ in $O(k)$, star $\operatorname{St}(a)$ in $O(|\operatorname{St}|)$, all $k$-simplices in $O(1)$.
...
\end{itemize}
The file \texttt{astrolabe.json} is the single source of truth. All plugins read from and write to this structure; no separate databases are used.

\subsection{Plugins}

Each plugin is a typed operation on the astrolabe file. Three types arise naturally:
\begin{itemize}
\item \emph{Source} ($X \to \mathbb{A}$): a parser that converts external data into astrolabe entries. Examples: \texttt{ilean\_parser} (Lean~4 compilation artifacts), BibTeX importer, LLM extraction. Each source plugin is a parsing procedure $P : A^* \to \mathcal{P}_{\mathrm{fin}}(R)$ followed by $h_{\mathrm{id}}$ assignment.
\item \emph{Endo} ($\mathbb{A} \to \mathbb{A}$): a transformation that modifies the astrolabe file. Enrichments (Definition~\ref{def:enrichment}) are the mildest form; structure-preserving maps and quotients are stronger. Examples: \texttt{math\_domain} (field defaults), \texttt{timestamp} (temporal metadata), \texttt{network\_analysis} (graph metrics), \texttt{llm\_embedding} (semantic hash $h_{\mathrm{emb}}$).
\item \emph{Sink} ($\mathbb{A} \to Y$): a query or rendering that reads the astrolabe and produces output (HTML, numerical invariants, visualizations). Cohomology (Definition~\ref{def:cohomology}) is a sink: $H^k$ maps the chain complex to abelian groups.
\end{itemize}
Composition of plugins follows the pipeline $\text{Source} \to \text{Endo} \to \cdots \to \text{Endo} \to \text{Sink}$. The astrolabe file is the single intermediate representation at every stage; no format conversion is needed between plugins.

\paragraph{Merging astrolabe files.}
Merging two astrolabe files $\mathbb{A}_1, \mathbb{A}_2$ requires no special operation: form the disjoint union $\mathbb{A}_1 \sqcup \mathbb{A}_2$ (a valid astrolabe file, since hashes are content-determined and do not collide), then add new entries whose \texttt{ref} fields draw from both hash pools. Each cross-reference is an ordinary simplex; the boundary operator, homology, and all invariants of \S\ref{sec:topology} apply to the merged file without modification. The change in Betti numbers tracks the structural effect of the merge: each cross-reference 1-simplex connecting the two components reduces $\beta_0$ by one; a cycle of cross-references increases $\beta_1$ by one.

\subsection{Boundary Computation}

The restriction map $d_i(\sigma)$ removes $\texttt{ref}[i]$ from the vertex list. The boundary operator is computed directly on the \texttt{ref} lists:
\[
...
2D force-directed graph visualization. Currently running on the 1-skeleton. Higher-dimensional analysis is supported by the \texttt{astrolabe.json} format: adding a parser that emits 2-simplices or higher requires no changes to the data model.
...
\medskip
\noindent\textbf{Unified structure (\texttt{astrolabe.json}).} All plugins operate on the same in-memory structure. The same task requires $p$ computation steps and $0$ data conversion steps, for a total of $p$ steps. With $t$ iterations, the total is $tp$.
\end{proposition}
...
\medskip
\noindent\textbf{Unified annotation.} All annotation functions are projections of a single function $F : S \to \texttt{astrolabe.json}$:
\[
