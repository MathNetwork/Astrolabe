...
This paper develops a mathematical framework that addresses these limitations from first principles. Starting from the observation that all structured knowledge reduces to finite strings over a finite alphabet, we construct \emph{semantic simplicial complexes}: simplices that carry hash identifiers, key-value content, and semantic coordinates (\S\ref{sec:foundations}). This construction is the foundation on which the rest of the theory is built.

The full machinery of algebraic topology then runs on these complexes. Homology groups count independent knowledge domains, circular arguments, and synergy gaps; cohomology captures non-conservative dependency flows; long exact sequences and the snake lemma give precise descriptions of what happens during version upgrades and modular refactoring; persistent homology tracks structural evolution across multiple filtrations (\S\ref{sec:topology}).

By equipping the complexes with LLM embeddings as a semantic metric, we prove an \emph{embedding obstruction theorem}: the topological complexity $k^*$ of a knowledge base gives a lower bound $d \geq k^*$ on embedding dimension. This yields formal characterizations of hallucination, scaling saturation, the distinction between understanding and memorization, and chain-of-thought reasoning (\S\ref{sec:semantics}).

The algebraic structure is organized categorically. Free categories, 2-categories, quotient/core/tower constructions, slices, sheaves, localization, and functionals provide the vocabulary for import, enrichment, rendering, and composition of knowledge structures (\S\ref{sec:category}).

The algebraic core of the theory is mechanically verified in Lean~4 with zero \texttt{sorry}. A reference implementation is provided as an open-source desktop application whose plugin architecture directly reflects the categorical structure: each plugin is a functor pair consisting of a backend computation component and a frontend rendering component (\S\ref{sec:implementation}).
