...
\node[bp] (thm) at (1.5, 2) {Theorem A};
\node[bp] (lem1) at (0, 0.5) {Lemma B};
\node[bp] (lem2) at (3, 0.5) {Lemma C};
\node[bp] (defn) at (1.5, -0.8) {Definition D};
\draw[arr] (thm) -- node[left, font=\tiny] {uses} (lem1);
\draw[arr] (thm) -- node[right, font=\tiny] {uses} (lem2);
\draw[arr] (lem1) -- node[left, font=\tiny] {uses} (defn);
\draw[arr] (lem2) -- node[right, font=\tiny] {uses} (defn);

% Right: Declaration-level graph (more edges, finer-grained)
\node[font=\small\bfseries] at (7.5, 2.8) {Declaration graph};
\node[decl, label={[font=\scriptsize]above:A}] (a) at (7.5, 2) {};
\node[decl, label={[font=\scriptsize]left:B}] (b) at (6, 0.8) {};
\node[decl, label={[font=\scriptsize]right:C}] (c) at (9, 0.8) {};
\node[decl, label={[font=\scriptsize]below:D}] (d) at (7.5, -0.5) {};
\node[decl, label={[font=\scriptsize]left:B.aux}] (baux) at (5.2, -0.2) {};
\node[decl, label={[font=\scriptsize]right:C.rec}] (crec) at (9.5, -0.2) {};
\node[decl, label={[font=\scriptsize]below:D.mk}] (dmk) at (7.5, -1.3) {};
\draw[darr] (a) -- (b);
\draw[darr] (a) -- (c);
\draw[darr] (b) -- (d);
\draw[darr] (c) -- (d);
...
\draw[darr] (crec) -- (dmk);
\draw[darr] (d) -- (dmk);
\draw[darr, dashed, gray] (a) to[bend right=20] (d);
\end{tikzpicture}
\caption{Left: a \texttt{leanblueprint} dependency graph. Each edge is labeled ``uses'' via \texttt{\textbackslash uses\{\}} macros. Right: the actual declaration-level dependency graph extracted from Lean compilation artifacts~\cite{LiPengSeverini2026}. The declaration graph has more nodes (auxiliary lemmas, recursors, constructors) and more edges, but both graphs lose the semantic content of each dependency: \emph{why} does A depend on B?}
