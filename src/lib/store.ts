...
import { checkProjectStatus } from '@/lib/api'
...
export const useStore = create<UIState>((set, get) => ({
  ...
  // Load project (knowledge-only - no Lean data to load)
  loadProject: async (path: string) => {
    set({ loading: true, error: null, projectPath: path })

    try {
      await get().checkStatus(path)
      // Knowledge data is loaded via canvasStore, not here
      set({ projectData: { path, nodes: [], edges: [], stats: { total_nodes: 0, total_edges: 0, by_kind: {}, by_status: {} } }, loading: false })
      console.log('[store] Project path set (knowledge-only):', path)
    } catch (e) {
