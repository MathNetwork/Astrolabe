...
/**
 * Undoable create knowledge node
 */
export async function createKnowledgeNodeUndoable(
  position?: { x: number; y: number; z: number }
): Promise<import('@/lib/api').KnowledgeNode | null> {
  const store = getCanvasStore()

  // Mutable closure to track the created node
  ...
  const command = {
    ...
    timestamp: Date.now(),
    do: async () => {
      const node = await store.addKnowledgeNode(position)
      if (node) {
        createdNode = node
      }
    },
    undo: async () => {
      if (createdNode) {
        await store.removeKnowledgeNode(createdNode.id)
      }
    },
  }
    ...
    async () => {
      if (node) {
        ...
        for (const edge of connectedEdges) {
          await store.addKnowledgeEdge(edge.source, edge.target, edge.sort)
        }
      }
    }
  )
