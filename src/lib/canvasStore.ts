...
interface CanvasState {
  ...
  knowledgeNodes: KnowledgeNode[]  // Knowledge graph nodes
  knowledgeEdges: KnowledgeEdge[]  // Knowledge graph edges
...
export const useCanvasStore = create<CanvasState>((set, get) => ({
  ...
  deleteNodeWithMeta: async (nodeId) => {
    ...
    try {
      ...
      if (isKnowledgeNode) {
        ...
        set({ knowledgeNodes: filteredKNodes, knowledgeEdges: filteredKEdges })
        console.log('[CanvasStore] Deleted knowledge node:', nodeId)
      }
  ...
  addKnowledgeNode: async (position) => {
    const { projectPath, knowledgeNodes } = get()
    if (!projectPath) return null
    ...
    try {
      ...
      console.log('[CanvasStore] Created knowledge node:', node.id)
      return node
    } catch (e) {
      console.error('[CanvasStore] Create knowledge node failed:', e)
      return null
    }
  },

  removeKnowledgeNode: async (nodeId) => {
    const { projectPath, knowledgeNodes, knowledgeEdges } = get()
    if (!projectPath) return
    ...
    try {
      ...
      set({
        ...
        knowledgeEdges: knowledgeEdges.filter(e => e.source !== nodeId && e.target !== nodeId),
      })
  ...
  resetAllData: async () => {
    ...
    try {
      // Delete entire data directory, forcing re-parse from .ilean
      ...
      console.log('[CanvasStore] Reset all data (deleted project data, will re-parse on reload)')
    } catch (e) {
