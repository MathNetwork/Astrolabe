 ...
 *
 * Based on NetMath desktop implementation, simplified version
...
function simulateStep(
  ...
  if (physics.clusteringEnabled && namespaceGroups) {
    ...
    // Note: namespaceGroups values are NetMathNode[], not string[]
...
function warmupSimulation(
  ...
  if (physics.clusteringEnabled) {
    const graphNodes = nodes.map(n => ({ ...n, name: n.name || n.id }))
    namespaceGroups = groupNodesByNamespace(graphNodes as any, physics.clusteringDepth)
  }
  ...
  if (physics.adaptiveSpringEnabled) {
    ...
    nodeDegrees = calculateNodeDegrees(graphNodes as any, graphEdges as any)
  }
...
export function ForceLayout({
  ...
  const namespaceGroups = useMemo(() => {
    ...
    const graphNodes = nodes.map(n => ({ ...n, name: n.name || n.id }))
    return groupNodesByNamespace(graphNodes as any, physics.clusteringDepth)
  ...
  const nodeDegrees = useMemo(() => {
    ...
    const graphNodes = nodes.map(n => ({ ...n, name: n.name || n.id }))
    const graphEdges = edges.map(e => ({ ...e }))
    return calculateNodeDegrees(graphNodes as any, graphEdges as any)
