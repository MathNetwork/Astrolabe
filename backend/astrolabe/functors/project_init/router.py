...
_signature_stores: dict = None


def set_stores(stores: dict):
    global _signature_stores
    _signature_stores = stores
...
async def reset_project(path: str = Query(..., description="Project path")):
    ...
    if _signature_stores is not None and path in _signature_stores:
