...
class TestObjMetadata:
    ...
    def test_create_obj_without_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            store = _make_store(Path(tmp))
            obj = store.create_obj(name="Test", sort="theorem")
    ...
    def test_update_obj_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            store = _make_store(Path(tmp))
            obj = store.create_obj(name="Test", sort="theorem")
            updated = store.update_obj(obj["id"], metadata={"score": 0.5})
            assert updated["metadata"]["score"] == 0.5

    def test_update_obj_metadata_overwrite(self):
        """update_obj overwrites metadata (field-agnostic, no special merge)."""
        with tempfile.TemporaryDirectory() as tmp:
            store = _make_store(Path(tmp))
            obj = store.create_obj(
                name="Test", sort="theorem",
            ...
            updated = store.update_obj(obj["id"], metadata={"b": 3, "c": 4})
            assert updated["metadata"] == {"b": 3, "c": 4}  # simple overwrite
...
class TestMorMetadata:
    ...
    def test_update_mor_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            n2 = store.create_obj(name="B", sort="definition")
            mor = store.create_mor(source=n1["id"], target=n2["id"])
            updated = store.update_mor(mor["id"], metadata={"curvature": -0.3})
