...
### Phase 2：/prove + /sorry 命令 ✅

| 完成项 | 说明 |
|--------|------|
| `/prove` | 3 阶段 workflow：收集网络上下文 → 委派 lean-prover agent → sync store |
| `/sorry` | 调 lean_sorry_scan 列出所有 sorry |
| `/sync-lean` | 调 lean_sync_state 同步 Lean 状态到 store |
| `search` MCP tool | 关键词搜索 entries（title/notes/content），4 个测试通过 |

### Phase 3：lean_sync_state ✅

| 完成项 | 说明 |
|--------|------|
| `lean_sync_state` MCP tool | 解析 .lean 声明 → 匹配 store atoms → 更新 state（proven/sorry/checked） |
| 测试 | 3 个新测试（39 总计）：state 正确、well-formedness 保持、无 Lean 项目报错 |
| Hessenberg 验证 | 37 声明匹配，所有 atoms 已有正确 state（0 updates） |

### Phase 4：Lean Prover Agent ✅

| 完成项 | 说明 |
|--------|------|
| Agent 定义 | `~/.claude/agents/lean-prover.md`：Session Contract + 策略 + 编译指令 |
| `/prove` 升级 | 改为 AgentTool 委派，主 agent 收集上下文，sub-agent 操作 Lean |
| 上下文验证 | `/prove rigid` 正确收集 theorem + tex hint + sorry 位置 + Lean root |

### Phase 5：网络指导 Proving ✅

| 完成项 | 说明 |
|--------|------|
| `/smart-prove` | 自动选 PageRank 最高目标，收集 propagation 影响 + cross_source hint + 依赖 |
| `/batch-prove` | 按 skeleton 拓扑排序批量证明，逐个 sync，最终汇总报告 |
| Hessenberg 验证 | `/smart-prove` 选中 rigid（5 downstream atoms），tex hint 正确传递 |
